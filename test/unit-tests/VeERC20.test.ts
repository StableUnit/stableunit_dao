...
describe("VeERC20", () => {
    ...
    beforeEach(async function () {
        ...
        veERC20 = (await deployProxy(
            "VeERC20",
            [accessControlSingleton.address, suDAO.address, await latest()],
            undefined,
            false
        )) as VeERC20;
    ...
    describe("availableToClaim", async () => {
        it("should vest right amount of tokens", async () => {
            const amountToLockInternal = BN_1E18.mul(123);
            await mintAndLockTokens(amountToLockInternal);
            ...
            // should be as amountToLockInternal.div(3) +- 3%
            expect(availableToClaim3).to.be.gt(amountToLockInternal.div(3).mul(97).div(100));
            expect(availableToClaim3).to.be.lt(amountToLockInternal.div(3).mul(103).div(100));

            await increaseTime(vestingPeriodSeconds);
            const availableToClaim4 = await veERC20.availableToClaim(user2.address);
            expect(availableToClaim4).to.be.equal(amountToLockInternal);
        });
    });

    describe("claim", async () => {
        it("can't claim 0 before cliff", async () => {
            const amountToLockInternal = BN_1E18.mul(100);
            await mintAndLockTokens(amountToLockInternal);
            // should have 0 available to claim
        ...
        it("claim all after vesting", async () => {
            ...
            expect(amountToLockInternal).to.be.equal(balanceAfter.sub(balanceBefore));
        });
