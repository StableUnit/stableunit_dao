...
describe("VeERC20", () => {
    ...
    beforeEach(async function () {
        ...
        veERC20 = (await deployProxy("VeERC20", [
    ...
    describe("availableToClaim", async () => {
        it("should vest right amount of tokens", async () => {
            const lockedAmount = BN_1E18.mul(123);
            await mintAndLockTokens(lockedAmount);
            ...
            // should be as lockedAmount.div(3) +- 3%
            expect(availableToClaim3).to.be.gt(lockedAmount.div(3).mul(97).div(100));
            expect(availableToClaim3).to.be.lt(lockedAmount.div(3).mul(103).div(100));

            await increaseTime(vestingPeriodSeconds);
            const availableToClaim4 = await veERC20.availableToClaim(user2.address);
            expect(availableToClaim4).to.be.equal(lockedAmount);
        });
    });
    ...
    describe("rescue", async () => {
        ...
        it("donate token to invalid DAO address should fail", async () => {
            await mintAndLockTokens();
            await expect(veERC20.connect(user2).donateTokens(user1.address)).to.be.revertedWith("invalid DAO address");

            await expect(veERC20.connect(user2).donateTokens(deployer.address)).to.be.revertedWith(
                "invalid DAO address"
            );
        });
    });
    ...
    describe("transfer", async () => {
        it("can't transfer veSuDAO", async () => {
            await expect(veERC20.transfer(user1.address, BN_1E18)).to.be.revertedWith(
                "not possible to transfer vested token"
            );
        });
    });
    ...
    describe("updateTge", async () => {
        it("can't update to the TGE_MAX_TIMESTAMP", async () => {
            await expect(veERC20.connect(admin).updateTgeTimestamp((await latest()) - 100)).to.be.revertedWith(
                "veERC20: TGE date can't be in the past"
            );
            await expect(veERC20.connect(admin).updateTgeTimestamp((await latest()) + 100)).to.be.revertedWith(
                "veERC20: new TGE date is beyond limit"
            );
        });
    });
});
